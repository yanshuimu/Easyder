...
- (void)configTextField{
    ...
    [NSLayoutConstraint activateConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"V:|-(h)-[_tableView]-0-|" options:0 metrics:@{@"h":@((KIsiPhoneX?44:20) + 44)} views:NSDictionaryOfVariableBindings(_tableView)]];
    [_tableView reloadData];
}

- (void)configClearBtn{

  _clearBtn = [[UIButton alloc]initWithFrame:CGRectMake([UIScreen mainScreen].bounds.size.width - 80, KIsiPhoneX?44:20, 60, 44)];
...
- (void)configSaveBtn{
    _saveBtn = [[UIButton alloc]initWithFrame:CGRectMake(CGRectGetMinX(_clearBtn.frame) - 70, KIsiPhoneX?44:20, 60, 44)];
...
- (void)configReadBtn{
    _readLogBtn = [[UIButton alloc]initWithFrame:CGRectMake(CGRectGetMinX(_saveBtn.frame) - 70, KIsiPhoneX?44:20, 60, 44)];
...
- (void)configMinimizeBtn{
    _minimize = [[UIButton alloc]initWithFrame:CGRectMake(20, KIsiPhoneX?44:20, 80, 44)];
...
- (void)saveText {
    if (self.logStingArray.count<1) {
        ...
        NSData *data = [NSJSONSerialization dataWithJSONObject:self.logStingArray options:NSJSONWritingPrettyPrinted error:nil];
        [[NSUserDefaults standardUserDefaults] setObject:data forKey:@"textSaveKey"];
        [[NSUserDefaults standardUserDefaults] synchronize];
    }
}
